...
- `enable_permissive_configuration` (Boolean) Only applicable when `provider_type` is GitHub or GitHubEnterprise; for any other `provider_type` the attribute is ignored and a warning is emitted. When set, Allows authentication without any restrictions. For security best practices, it is recommended to add restrictions to limit access and enforce stricter controls. Use with caution, as this may grant broader access.
