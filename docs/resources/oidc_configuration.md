...
- `enable_permissive_configuration` (Boolean) Only enforced when `provider_type` is `GitHub`. The JFrog platform ignores this attribute for `GitHubEnterprise` (where it is sent but not applied), and for `generic` and `Azure` (where it is not sent at all), reporting permissive authentication as enabled regardless; for those provider types a plan-time warning is emitted and Terraform state will not reflect the platform. When set, Allows authentication without any restrictions. For security best practices, it is recommended to add restrictions to limit access and enforce stricter controls. Use with caution, as this may grant broader access.
