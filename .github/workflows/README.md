...
After you approve, tests **do** run the customer SHA with Artifactory license and Slack secrets, and they start an Artifactory container. That is required to test their code. Treat the approval as a trust decision: do not approve a PR that looks like it will dump env or exfiltrate credentials.
...
A PR may already add a `CHANGELOG.md` bullet for the resource or fix. That is a normal PR file. **Merging the PR lands that bullet.** CI does not write it.

### 2. CI “Tested on Artifactory …” heading (we cannot push to a fork)

The old job rewrote the version heading and `git push`ed to the PR branch. `GITHUB_TOKEN` cannot write to `customer/terraform-provider-platform`.

| PR type | After tests pass |
|---|---|
| Same-repo (`jfrog/terraform-provider-platform`) | CI still updates `CHANGELOG.md` and pushes, as before |
...
3. Terraform and OpenTofu acceptance tests run (Helm + Artifactory container, same as before).
...
These must be done on `jfrog/terraform-provider-platform` after this lands on `main`:
...
| `slack-notify-issues.yml` | issues | Slack notify only (unchanged) |
| `release.yml` | tag `v*` | Release (unchanged) |
| `changelog.yml` | changelog automation | Unchanged |
