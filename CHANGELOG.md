...
* resource/platform_global_role: Removed the hard-coded allow-list validation for the `actions` attribute. This allows importing and managing predefined global roles that use actions not previously listed in the provider (e.g. `READ_APPLICATION`, `READ_APPLICATION_VERSION`, `READ_APPTRUST_POLICY`). The JFrog platform API remains the authoritative source for valid action values. Issue: [#332](https://github.com/jfrog/terraform-provider-platform/issues/332) PR: [#338](https://github.com/jfrog/terraform-provider-platform/pull/338)
